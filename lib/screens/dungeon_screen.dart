...
class _DungeonScreenState extends State<DungeonScreen>
  ...
  void _enterDungeon(Map<String, dynamic> dungeon) async {
    ...
    if (power < required) {
      ...
      widget.gameState.energy += GameState.dungeonEnergyCost; // 환불
      widget.onStateChanged();
      return;
    }
          ...
          child: Column(
            children: [
              Row(
                ...
                children: [
                  Column(children: [
                    const Text('⚡ 전투력', style: TextStyle(color: Colors.grey, fontSize: 11)),
                    Text(_formatNumber(power),
                        style: const TextStyle(color: Color(0xFFF5C842), fontSize: 22, fontWeight: FontWeight.bold)),
                  ]),
                  Column(children: [
                    const Text('🔋 에너지', style: TextStyle(color: Colors.grey, fontSize: 11)),
                      ...
                      children: [
                        AnimatedBuilder(
                          ...
                          builder: (context, child) => Transform.translate(
                            offset: Offset(_battleStage == 4 ? _shakeAnim.value : 0, 0),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final canEnter = powerOk && gs.canEnterDungeon && !_isClearing;
              final clearTimeMs = _calcClearTimeMs(power, required);
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  color: const Color(0xFF111118),
                  border: Border.all(
                    color: powerOk ? const Color(0xFF2A4A2A) : const Color(0xFF2A2A3A),
                  ),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: ListTile(
                  leading: Text(dungeon['emoji'], style: const TextStyle(fontSize: 28)),
    ...
    Text(
      ...
      style: const TextStyle(color: Colors.grey, fontSize: 10),
                  ...
                  trailing: ElevatedButton(
                    ...
                    child: Text(
                      _isClearing ? '...' : !powerOk ? '🔒' : !gs.canEnterDungeon ? '⚡부족' : '입장',
